...
ui <- fluidPage(	
  ...
  navbarPage("Student dataset",	
             tabsetPanel(type = "tabs",	
                         ...
                         tabPanel("parent's occupation vs. avg grade of the students by age",	
                         ...
                         tabPanel("Health vs. Average Grades with and without Academic Support",	
                                  sidebarLayout(	
                                    sidebarPanel(	
...
server <- function(input, output) {
  ...
  output$kayplot <- renderPlot ({
    ...
    ggplot(avg_grade, aes(Mjob, avg)) +
      geom_col() + 
      labs(title = "Mother's occupation vs. avg grade of the students by age",
           x = "Mother's job", y = "Average grades")
  })
  output$kayplot2 <- renderPlot ({
    ...
    ggplot(grade_avg, aes(Fjob, avg)) +
      ...
      labs(title = "Father's occupation vs. avg grade of the students by age",
           x = "Father's job", y = "Average grades")
